...
@Gooey(
    program_name="DataTag Helper (標註轉檔小幫手) v0.8.7",
    navigation="TABBED",
    tabbed_groups=False,
...
def split_train_test_to_target(X, y, target):
    ...
    for index, (train_index, test_index) in enumerate(sss.split(X, y)):
        ...
        with pd.ExcelWriter(
            filename, options={"strings_to_urls": False}, mode=mode, engine="openpyxl"
        ) as writer:
            df_target_train.to_excel(
                writer, sheet_name="train{:02}".format(index), index=False
            )
            df_target_test.to_excel(
                writer, sheet_name="test{:02}".format(index), index=False
            )
...
def concat_files(files_list):
    ...
    df_document_label = pd.read_excel(
        files_list[0], sheet_name="document_label", engine="openpyxl"
    )
    df_sentence_label = pd.read_excel(
        files_list[0], sheet_name="sentence_label", engine="openpyxl"
    )

    for filepath in files_list[1:]:
        df_content = df_content.append(
            pd.read_excel(filepath, sheet_name="contents", engine="openpyxl")
        )
        df_document_label = df_document_label.append(
            pd.read_excel(filepath, sheet_name="document_label", engine="openpyxl")
        )
        df_sentence_label = df_sentence_label.append(
            pd.read_excel(filepath, sheet_name="sentence_label", engine="openpyxl")
        )
    ...
    df_document_label = df_document_label.applymap(
        lambda x: unescape_OOXML(x) if isinstance(x, str) else x
    )
    df_sentence_label = df_sentence_label.applymap(
        lambda x: unescape_OOXML(x) if isinstance(x, str) else x
    )
    ...
    with pd.ExcelWriter(
        "all_data.xlsx", options={"strings_to_urls": False}, engine="openpyxl"
    ) as writer:
        df_content.to_excel(writer, sheet_name="contents", index=False)
        df_document_label.to_excel(writer, sheet_name="document_label", index=False)
        df_sentence_label.to_excel(writer, sheet_name="sentence_label", index=False)
    ...
    if args.command == "original":
        if args.input_file:
            ...
            if args.emojilize:
                # df = clean_data(df)
                df = text_to_emoji(df)
                new_filename = common_filename.with_name(
                    common_filename.stem + "_emojilized"
                )
                print(args.input_file.split("."))
            else:
                new_filename = common_filename.with_name(
                    common_filename.stem + "_demojilized"
                )
            ...
            df_remove_emoji_text = df.applymap(
            ...
            id_mapping_filename = common_filename.with_name(
                common_filename.stem + "_TextID_mapping"
            ).with_suffix(".xlsx")
            with pd.ExcelWriter(
                id_mapping_filename,
                options={"strings_to_urls": False},
        ...
        save_path = (
            Path(common_filename)
            .with_name(common_filename.stem + "_second_upload")
            .with_suffix(".json")
        )
        ...
        save_path = (
            Path(common_filename_2)
            .with_name(common_filename_2.stem + "_second_labeled")
            .with_suffix(".json")
        )
        ...
        df_document = pd.read_excel(
            args.input_file, sheet_name="document_label", engine="openpyxl"
        )
        df_sentence = pd.read_excel(
            args.input_file, sheet_name="sentence_label", engine="openpyxl"
        )

        ## unescape OOXML string
        # df_content = df_content.applymap(lambda x: unescape_OOXML(x) if isinstance(x, str) else x)
        df_document = df_document.applymap(
            lambda x: unescape_OOXML(x) if isinstance(x, str) else x
        )
        df_sentence = df_sentence.applymap(
            lambda x: unescape_OOXML(x) if isinstance(x, str) else x
        )
